...
class ReservoirCapacityChange(QgsProcessingAlgorithm):
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterFile("INPUT_CSV", "Initial Elevation-Capacity CSV", extension="csv"))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for i in range(1, len(df1)):
            ...
            if i == 1:
                ...
                df1.at[i, "Area (m²)"] = 1.5 * volume / elevation_diff
            else:
        ...
        heightt= (np.nanmax(raster_array2))-(np.nanmin(raster_array2))
        
        geotransform2 = dataset2.GetGeoTransform()

        # Get raster size in pixels
        xsize2 = dataset2.RasterXSize  # Number of columns
        ysize2 = dataset2.RasterYSize  # Number of rows
        
        # Get pixel size in meters
        pixel_width2 = geotransform2[1]   # Pixel width in meters
        pixel_height2 = abs(geotransform2[5])  # Pixel height in meters (absolute value)

        # Convert to kilometers
        raster_length = (ysize2 * pixel_height2)
        raster_breadth = (xsize2 * pixel_width2)
        
        raster_length, raster_breadth = (
        (raster_length / 2, raster_breadth) if raster_length < raster_breadth 
        else (raster_length, raster_breadth / 2))
        
        # Compute sec(θ) for raster_length
        hypotenuse_length = np.sqrt(heightt**2 + raster_length**2)
        sec_theta_length = hypotenuse_length / raster_length

        # Compute sec(θ) for raster_breadth
        hypotenuse_breadth = np.sqrt(heightt**2 + raster_breadth**2)
        sec_theta_breadth = hypotenuse_breadth / raster_breadth

        # Multiply sec(θ) values
        sec_theta_product = sec_theta_length * sec_theta_breadth
        ...
        for i in range(len(df_combined) - 1):
            ...
            if A1 != 0 and A2 != 0:
                A1= abs(A1)* sec_theta_product
                A2= abs(A2)* sec_theta_product
                A_mid = (A1 * A2) ** 0.5  # Geometric mean of A1 and A2
                Depth_Difference = 1*(3 * SED) / (A1 + 1 * A_mid + A2)
            elif A1 == 0 or A2 == 0:
                A1= A1* sec_theta_product
                A2= A2* sec_theta_product
                A_mid = (A1 * A2) ** 0.5  # Geometric mean of A1 and A2
                Depth_Difference = 3* (SED) / (A1 + 1 * A_mid + A2)
            else:
        ...
        #new_df.to_csv(output_combined_csv, index=False)
